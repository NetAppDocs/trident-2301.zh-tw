...
| `autoExportPolicy` | 啟用自動匯出原則建立及更新[布 林值]。使用 `autoExportPolicy` 和 `autoExportCIDRs` 選項：Astra Trident可自動管理匯出原則。 | 錯 
...
.預設選項開啟 <code>ontap-nas-economy</code>
[%collapsible%open]
====
[listing]
----
...
nfsMountOptions: nfsvers=4
----
.<code>ontap-nas-flexgroup</code> 驅動程式
[listing]
----
---
version: 1
...
----
====
.<code>ontap-nas-economy</code> 驅動程式
[%collapsible%open]
====
[listing]
----
